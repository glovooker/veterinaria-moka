   ...
   //validar los campos SELECT 
    for(var i = 0; i < selects.length; i++)  {
        if (selects[i].required == true) {
            if (selects[i].value == "" || selects[i].value == null || selects[i].value == undefined){
                ImprimirMsjError('Seleccione un valor para: '+ lblSelects[i].textContent);            
    ...
    for(var i = 0; i < fieldSetList.length; i++) { 
        ...
        for(var j = 0; j < fieldSetList[i].children.length; j++) {  
            ...
            //valida radios 
            if (typeElement != null && typeElement != undefined && typeElement.toLowerCase() == 'radio'){
                tipoElemento = 'radio';
            ...
            //valida checkbox
            if (typeElement != null && typeElement != undefined && typeElement.toLowerCase() == 'checkbox'){
        ...
        if ((tipoElemento == 'checkbox' && minCks > 0) && (ckSel == false || (cantCks < minCks))) {
    ...
    for(var i = 0; i < textAreas.length; i++)  {
        if (textAreas[i].required == true) {
            if (textAreas[i].value == "" || textAreas[i].value == null || textAreas[i].value == undefined){
                ImprimirMsjError('Indique un valor para: '+ lblTextAreas[i].textContent);            
...
function ResaltarLabelInvalido(pLabelId){
    ...
    setTimeout( function () {
        elementLabel.style = styleOrigin;}, 5000);
}
