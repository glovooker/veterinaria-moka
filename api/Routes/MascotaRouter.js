...
router.post('/RegistrarMascota', (req, res)=>{
    ...
    nuevaMascota.save((err, mascotaDB)=>{
        if(err){
            res.json({
            resultado: false,
            msj: "Error al registrar la mascota : ", err
            });
        }else{
            res.json({
            resultado: true,
            msj: "Registro realizado de manera correcta", mascotaDB
            });   
        }
    });
});
...
router.get('/BuscarMascota', (req, res) => {
    ...
    } else if ((params.IdPersona != "" && params.IdPersona != null && params.IdPersona!= undefined) && (params.Nombre == "" || params.Nombre == null || params.Nombre == undefined)) {
...
router.post('/ModificarMascota', function(req, res){
    let body = req.body
    Mascota.updateOne({IdPersona:body.IdPersona}, {
        $set: req.body
    }, function (err, info){
        if(err){
            res.json({
...
});router.get('/BuscarIdPersona',(req, res)=>{
    let params = req.query;
    if (params.IdPersona != "" && params.IdPersona != null && params.IdPersona != undefined) {
        Mascota.findOne({IdPersona: params.IdPersona}, (err, IdPersonaBD) => {
        if(err){
            res.json({
                resultado: false,
                msj: 'No se pudo encontar la información', 
